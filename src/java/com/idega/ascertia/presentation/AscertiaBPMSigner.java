...
public class AscertiaBPMSigner extends Block {
	...
	protected Form addForm(IWContext iwc, Map<String, String> paramsMap) throws RemoteException {
		Form form = new Form();
	...
	protected void showSignedPdf(IWContext iwc) throws RemoteException {
		...
		mainDiv.add(PresentationUtil.getJavaScriptSourceLine(iwc.getIWMainApplication().getBundle(CoreConstants.CORE_IW_BUNDLE_IDENTIFIER)
	...
	protected void addFormParams(IWContext iwc, Form form, Map<String, String> paramsMap) throws RemoteException{
		...
		String successPath = builderService.getUriToObject(AscertiaBPMSigner.class,  Arrays.asList(new AdvancedProperty(PARAMETER_ACTION,String.valueOf(PARAMETER_SHOW_SIGNED_PDF))));

		String errorPath =builderService.getUriToObject(AscertiaBPMSigner.class,  paramList);
		
		addCertificatesList(form);
		Link sign = new Link(/*getLocalizedString("sign_document", "Sign document", iwc)*/"Sign document");
		sign.setURL("javascript:void(0)");
		sign.setOnClick("signDocument('" + successPath + "','" + errorPath + "','" + /*getLocalizedString("signing", "Signing...", iwc)*/ "Signing..." + "');");
		form.add(sign);
	}
